  ...
  return <>
      ...
      <div className={`min-h-screen bg-background flex flex-col items-center justify-center px-6 transition-opacity duration-800 ${fadeOut ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="text-center space-y-8 md:space-y-12">
          {/* Brand Name */}
          <h1 className={`font-heading text-5xl md:text-7xl lg:text-8xl font-black tracking-[0.2em] text-foreground transition-all duration-1000 ${textVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            BRANCA
          </h1>

          {/* Tagline */}
          <p className={`font-body text-sm md:text-base tracking-[0.3em] uppercase text-muted-foreground transition-all duration-1000 delay-300 ${taglineVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>NEW WEBSITE COMING SOON.</p>
